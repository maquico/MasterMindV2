...
void recogerCaracteres(Nodo*& lista, string ruta, float& contadorEspacios, float& contadorLetras, int &contadorPalabras)
{
    ifstream archivo(ruta.c_str());
    unsigned char caracter;
    bool valido = true, palabraRepetida = false;
    ...
    if (archivo.is_open())
    {
        while (archivo.good())
        {
            ...
            if (isalpha(caracter))
            {
            	if(palabraRepetida && int(caracter)!=255) palabraRepetida = false; //Comprueba que existe la palabra teórica 
            	else if(int(caracter==255)&& palabraRepetida) contadorPalabras--; //Comprueba que no detecte el caracter final como una palabra nueva luego de un char especial
            ...
            else if(!palabraRepetida && caracter!=' '){ //Hay un caracter especial despues de una letra, se suma una palabra teórica
            	contadorPalabras++;
            	palabraRepetida=true;
			}
			else if(caracter==' ' && palabraRepetida){ //Despues de uno o mas caracteres especiales hay un espacio, significa
														//que ya se habían contado aparte las palabras y se resta la palabra teórica
...
int main()
{
    ...
    recogerCaracteres(lista, ruta, espaciosTotales, letrasTotales, palabras);
    cout << "La cantidad de palabras en el archivo es: " <<  palabras << endl;
    Mostrar(lista);
    cout << endl;
